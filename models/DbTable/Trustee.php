...
class Application_Model_DbTable_Trustee extends Zend_Db_Table_Abstract{
    ...
    public function getMyTrustees($userId,$returnSelect = false){
        ...
        $select = $trusteeTable->select()
                        ...
                        ->joinLeft('usrSetting', 'usrSetting.userId = trustees.trusteeId',array('isOnline'))
    ...
    public function getIncomingOutgoingRequest($userId,$returnSelect = false,$search=null,$alpha=null){
        $trusteeTable = new Application_Model_DbTable_Trustee();
        ...
        // incoming trustee request 
         ...
         if ($search != null && $alpha == null) {
            $select1->where("(userNickName like ?", "%$search%")
                    ->orWhere("userFullName like ?)", "%$search%");
            $select2->where("(userNickName like ?", "%$search%")
                    ->orWhere("userFullName like ?)", "%$search%");
        }
        if ($search != null && $alpha != null) {
            $select1->where("userNickName LIKE '$search%'");
