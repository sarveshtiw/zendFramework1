...
class WaController extends My_Controller_Abstract{
   ...
   */
    ...
    */
    ...
    public function sendChatMessage($waRow,$dialog_id){
        $userTable        = new Application_Model_DbTable_Users();
        $waTable          = new Application_Model_DbTable_Wa();
        $waReceiverTable  = new Application_Model_DbTable_WaReceiver();
        ...
        foreach ($waReceiverRowset as $waReceiverRow)
        {
            if($waReceiverRow->receiver_quickblox_id){
               ...
               // $chat_dialog_id = $this->createDialogId($token,$waReceiverRow->receiver_quickblox_id);
                ...
                $data = array(
                        'chat_dialog_id' => $waReceiverRow->receiver_quickblox_id,
                        'message'        => $waRow->text,
    ...
    */
    ...
    public function quickBloxUserToken1(){
        $userEmail = "akhilesh12";
        $endpoint="https://api.quickblox.com/";
        $app_id ="9361";
        $auth_secret="Lqf9MaZjuce9Cen";
        $auth_key="h4GL7ABbpGVTBgV";
        $nonce = rand();
        $timestamp = time();
        $signature_string = "application_id=" .$app_id . "&auth_key=" . $auth_key . "&nonce=" . $nonce . "&timestamp=" . $timestamp . "&user[login]=" . $userEmail . "&user[password]=" ."akhilesh";
        ...
        $post_body = http_build_query(array(
            ...
            'user[password]'    => "akhilesh"
        ));
        
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $endpoint.$this->quickblox_details_new->api_path_session); // Full path is - https://api.quickblox.com/session.json
    ...
    public function staticsAction(){
      ...
      $result =  $userTable->getUserCount();

       $this->user->sendmail($result,'statics.phtml','akh@wa-app.com','WA-app Statics');
       $this->user->sendmail($result,'statics.phtml','iwan@wa-app.com','WA-app Statics');
       die();         
    }
}
